/**** update functions to correct the way of aggregating baseline aq, scenario aq, and delta aq.****/
UPDATE "data".settings SET value_int=35 WHERE "key"='version';
	...
	if (_source_grid_definition_id != _output_grid_definition_id) then
		...
		return query	
			SELECT 
       			...
       			sum(er.baseline_aq * ce.percentage * er.subgroup_population) / sum(er.subgroup_population * ce.percentage) as baseline_aq,
       			sum(er.scenario_aq * ce.percentage * er.subgroup_population) / sum(er.subgroup_population * ce.percentage) as scenario_aq,
       			sum(er.delta_aq * ce.percentage * er.subgroup_population) / sum(er.subgroup_population * ce.percentage) as delta_aq
  			FROM 
	...
	if (_source_grid_definition_id != _output_grid_definition_id) then
		...
		return query	
			SELECT 
       			...
       			sum(hr.baseline_aq * ce.percentage * hr.population) / sum(hr.population * ce.percentage) as baseline_aq,
       			sum(hr.scenario_aq * ce.percentage * hr.population) / sum(hr.population * ce.percentage) as scenario_aq,
       			sum(hr.delta_aq * ce.percentage * hr.population) / sum(hr.population * ce.percentage) as delta_aq,
