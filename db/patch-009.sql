-- Fix the AQ aggregation to properly apply population weighting

UPDATE "data".settings SET value_int=9 where "key"='version';
	...
	if (_source_grid_definition_id != _output_grid_definition_id) then
		...
		return query	
			SELECT 
       			...
       			sum(hr.baseline_aq * ce.percentage * hr.population) / sum(ce.percentage * hr.population) as baseline_aq,
       			sum(hr.scenario_aq * ce.percentage * hr.population) / sum(ce.percentage * hr.population) as scenario_aq,
       			sum(hr.delta_aq * ce.percentage * hr.population) / sum(ce.percentage * hr.population) as delta_aq,
