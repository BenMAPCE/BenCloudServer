...
UPDATE "data".settings SET value_int=84 where "key"='version';

--issue when default dataset were linked to user-uploaded groups. 
...
and not exists (select 1 from data.endpoint_group lu where lu.name = eg."name" and lu.user_id is null);
...
--Issue when user uploads have the same name as default
...
inner join data.endpoint_group eg2 on eg1.name=eg2."name" 
...
inner join data.health_impact_function_group hifg2 on hifg1.name=hifg2."name" 
